...
**NOTE**: You will need to render the markdown file inside the console with `rmarkdown::render("path_to_file/debugging.Rmd)` to have some functions show correct output.
...
3.  Try making the error / bug repeatable. Creating a **Repro**ducible **ex**ample helps you figure out the root cause of the problem, and helps others understand the exact problem you are running into. If possible, try to create an example with small and simple test cases, which can help think more directly about the problem.
...
**Question**: How can I create a scatter plot with data points colored based on a column? I've tried setting `color=` to the column name inside `geom_points()`, but I'm getting an error.
...
Once we identify the potential location of the bug, it's time to figure out what is happening. One common approach is to utilize the `print()` statement to print the value of objects during the execution.
...
Reusing the previous example, I will put print statements throughout the function to leave trails of `x`.
...
subset_data <- function(df, column_names) {
...
calculate_colmeans <- function(df, column_names) {
  ...
  return(colMeans(df_subset)) # add drop=FALSE option
}
...
add_one_to_factor <- function(x) {
  ...
  factor_to_integer <- as.integer(as.character(example_x))
...
In the following session, we will explore several powerful debugging options that allow you to "pause" the execution of a function and interactively explore.
...
Try inserting `browser()` inside `calculate_colmeans` and explore the function.
Note that you can choose to insert the `browser()` statement anywhere in the function, and it will start the interactive session at that place.
...
calculate_geometric_means <- function(x, columns) {
  for (col in columns) {
    x <- geometric_mean(x[,col])
    print(paste0(col, ": ", round(x, 4)))
  }
}
